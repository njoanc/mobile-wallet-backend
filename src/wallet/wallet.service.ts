...
export class WalletService {
  ...
  async fundWalletWithCard(user: any, data: FundWalletByCardDto) {
    ...
    if (funded.status === 'success') {
      funded.message = ` ${data.amount} tokens is on the way to your wallet, you can check your wallet to see the balance`;
  ...
  async fundWalletByBank(user: any, data: FundWalletByBanktDto) {
    ...
    if (fundedBank.status === 'success') {
      fundedBank.message = `${data.amount} tokens is on the way to your wallet, you can check your wallet to see the balance`;
  ...
  async withdrawFromWallet(user: any, data: WithdrawWalletDto) {
    ...
    if (withdrawal.status === 'success') {
      withdrawal.message = `You have successfully withdrawn ${data.amount} tokens from your wallet`;
  ...
  async transferPeerTokens(data: PeerTransferDto) {
    try {
      ...
      const transactionQueue: Transactions[] = [
        await this.transactionService.createTransaction(transactionObjSender),
        await this.transactionService.createTransaction(transactionObjReceiver),
      ];
