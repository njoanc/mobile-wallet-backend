...
export class UserService {
    ...
    async requestVerifyEmail(data: RequestVerifyEmailDto) {
        ...
        try {
            const checkEmail: Email = await this.EmailRepository.findOne({where:{
                email: data.email,
                valid: true,
            },
    ...
    async verifyEmail(data: VerifyEmailDto) {
        try {
            const findEmailVerifyToken: Email =
                await this.EmailRepository.findOne({where:{verifyToken: data.token,
                    valid: true,
                    email: data.email,}
    ...
    async resetPin(data: ResetPinDto) {
        try {
            const findUserToken: User = await this.UserRepository.findOne({where:{resetToken: data.token,}
