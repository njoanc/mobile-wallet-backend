...
export class UserService {
  ...
  async requestVerifyEmail(data: RequestVerifyEmailDto) {
    ...
    try {
      const checkEmail: Email = await this.EmailRepository.findOne({
        where: {
          email: data.email,
          valid: true,
        },
  ...
  async verifyEmail(data: VerifyEmailDto) {
    try {
      const findEmailVerifyToken: Email = await this.EmailRepository.findOne({
        where: { verifyToken: data.token, valid: true, email: data.email },
      });
  ...
  async updateTransactionPin(data: ChangeTransactionPinDto, id: string) {
    try {
      ...
      if (!(await Compare(data.oldPin, user.transactionPin)))
        throw new IncorrectCredentialsException('incorrect transaction pin!');
